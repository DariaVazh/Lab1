...
Описание: на вход подаются 4 целочисленных значения. Создается счетчик людей, которые смогли выкинуть мусор, с начальным значением 0. Проверяем вводимые данные на соответствие условию. Если данные некорректны, то вывести <Данные некорректны>.
...
1. Тест на проверку допустимости введенных значений
   * Input
      ```
     -5
     1
 ...
 Следующие тесты проверяют каждую ветвь блок схемы.

 2. Тест, в ходе которого все жильцы выбросят мусор
    * Input
    ```
    50
    5
...
3. Тест, в ходе которого никто не сможет выбросить мусор
   * Input
   ```
   5
   10
...
4. Тест, в ходе которого только 1 жилец выкинет мусор
   * Input
   ```
   10
   10
...
5. Тест, в ходе которого только 2 жилец выкинет мусор
   * Input
   ```
   15
   20
...
6. Тест, в ходе которого только 3 жилец выкинет мусор
   * Input
   ```
   15
   20
...
7. Тест, в ходе которого 1 и 2 жильцы выкинут мусор
   * Input
   ```
   20
   15
...
8. Тест, в ходе которого 2 и 3 жильцы выкинут мусор
   * Input
   ```
   20
   25
...
9. Тест, в ходе которого 1 и 3 жильцы выкинут мусор
   * Input
   ```
   20
   15
